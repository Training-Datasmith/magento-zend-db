...
class Zend_Db_Table extends Zend_Db_Table_Abstract
{
    ...
    public function __construct($config = [], $definition = null)
    {
        ...
        if (is_string($config)) {
            if (Zend_Registry::is_registered($config)) {
                trigger_error(self::class . '::' . __METHOD__ . '(\'registryName\') is not valid usage of Zend_Db_Table, ' . 'try extending Zend_Db_Table_Abstract in your extending classes.', E_USER_NOTICE);
                $config = [self::ADAPTER => $config];
            } else if ($definition instanceof Zend_Db_Table_Definition && $definition->has_table_config($config)) {
                // this will have DEFINITION_CONFIG_NAME & DEFINITION
                $config = $definition->get_table_config($config);
            } else {
