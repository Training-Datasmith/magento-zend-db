...
class Zend_Db_Adapter_Mysqli extends Zend_Db_Adapter_Abstract
{
     ...
     * @var string
     */
    protected $_default_stmt_class = 'Zend_Db_Statement_Mysqli';
    /**
     * Quote a raw string.
     *
     ...
     */
    public function get_quote_identifier_symbol(): string
    {
        return '`';
    }
    /**
    ...
    public function list_tables(): array
    {
        ...
        $sql = 'SHOW TABLES';
        if ($query_result = $this->get_connection()->query($sql)) {
            ...
            throw new Zend_Db_Adapter_Mysqli_Exception($this->get_connection()->error);
        }
        return $result;
    }
    /**
    ...
    public function describe_table($table_name, $schema_name = null): array
    {
         ...
         */
        if ($schema_name) {
            $sql = 'DESCRIBE ' . $this->quote_identifier("{$schema_name}.{$table_name}", true);
        } else {
            $sql = 'DESCRIBE ' . $this->quote_identifier($table_name, true);
        }
        /**
        ...
        $desc = [];
        $row_defaults = ['Length' => null, 'Scale' => null, 'Precision' => null, 'Unsigned' => null, 'Primary' => false, 'PrimaryPosition' => null, 'Identity' => false];
        $i = 1;
        $p = 1;
    ...
    protected function _connect()
    {
        ...
        $_is_connected = @mysqli_real_connect($this->_connection, $this->_config['host'], $this->_config['username'], $this->_config['password'], $this->_config['dbname'], $port, $socket);
        if ($_is_connected === false || mysqli_connect_errno()) {
