...
abstract class Zend_Db_Adapter_Abstract
{
     ...
     * @var string
     */
    protected $_default_stmt_class = 'Zend_Db_Statement';
    /**
     * Default class name for the profiler object.
     *
     * @var string
     */
    protected $_default_profiler_class = 'Zend_Db_Profiler';
    /**
     * Database connection
     *
     ...
     * @var integer
     */
    protected $_case_folding = Zend_Db::CASE_NATURAL;
    /**
     ...
     * @var bool
     */
    protected $_auto_quote_identifiers = true;
    /**
     ...
     */
    protected $_numeric_data_types = [Zend_Db::INT_TYPE => Zend_Db::INT_TYPE, Zend_Db::BIGINT_TYPE => Zend_Db::BIGINT_TYPE, Zend_Db::FLOAT_TYPE => Zend_Db::FLOAT_TYPE];
    /** Weither or not that object can get serialized
     *
     * @var bool
     */
    protected $_allow_serialization = true;
    /**
     ...
     * @var bool
     */
    protected $_auto_reconnect_on_unserialize = false;
    /**
     * Constructor.
     *
    ...
    public function __construct($config)
    {
        ...
        $this->_check_required_options($config);
        $options = [Zend_Db::CASE_FOLDING => $this->_case_folding, Zend_Db::AUTO_QUOTE_IDENTIFIERS => $this->_auto_quote_identifiers, Zend_Db::FETCH_MODE => $this->_fetch_mode];
        $driver_options = [];
        /*
        ...
        if (array_key_exists(Zend_Db::FETCH_MODE, $options)) {
            ...
            $this->set_fetch_mode((int) $options[Zend_Db::FETCH_MODE]);
        }
        // obtain quoting property if there is one
        if (array_key_exists(Zend_Db::AUTO_QUOTE_IDENTIFIERS, $options)) {
            $this->_auto_quote_identifiers = (bool) $options[Zend_Db::AUTO_QUOTE_IDENTIFIERS];
        }
        // obtain allow serialization property if there is one
        if (array_key_exists(Zend_Db::ALLOW_SERIALIZATION, $options)) {
            $this->_allow_serialization = (bool) $options[Zend_Db::ALLOW_SERIALIZATION];
        }
        ...
        if (array_key_exists(Zend_Db::AUTO_RECONNECT_ON_UNSERIALIZE, $options)) {
            $this->_auto_reconnect_on_unserialize = (bool) $options[Zend_Db::AUTO_RECONNECT_ON_UNSERIALIZE];
        }
    ...
    {
        $enabled = null;
        $profiler_class = $this->_default_profiler_class;
        ...
        } elseif (!$profiler_is_object) {
            $enabled = (bool) $profiler;
        }
        if ($profiler_instance === null) {
            if (!class_exists($profiler_class)) {
                #require_once 'Zend/Loader.php';
                Zend_Loader::load_class($profiler_class);
            }
            $profiler_instance = new $profiler_class();
        }
        if (!$profiler_instance instanceof Zend_Db_Profiler) {
            /** @see Zend_Db_Profiler_Exception */
            #require_once 'Zend/Db/Profiler/Exception.php';
            throw new Zend_Db_Profiler_Exception('Class ' . get_class($profiler_instance) . ' does not extend ' . 'Zend_Db_Profiler');
        }
        if (null !== $enabled) {
            $profiler_instance->set_enabled($enabled);
        }
        $this->_profiler = $profiler_instance;
        return $this;
    }
    /**
    ...
    {
        $this->_default_stmt_class = $class;
        return $this;
    }
    /**
    ...
    {
        $this->_connect();
        $q = $this->_profiler->query_start('begin', Zend_Db_Profiler::TRANSACTION);
    ...
    {
        $this->_connect();
        $q = $this->_profiler->query_start('commit', Zend_Db_Profiler::TRANSACTION);
        $this->_commit();
        $this->_profiler->query_end($q);
        return $this;
    }
    ...
    {
        $this->_connect();
        $q = $this->_profiler->query_start('rollback', Zend_Db_Profiler::TRANSACTION);
        $this->_roll_back();
        $this->_profiler->query_end($q);
        return $this;
    }
    ...
    public function insert($table, array $bind)
    {
        ...
        foreach ($bind as $col => $val) {
            ...
            if ($val instanceof Zend_Db_Expr) {
                $vals[] = $val->__toString();
                unset($bind[$col]);
            } else if ($this->supports_parameters('positional')) {
                $vals[] = '?';
            } else if ($this->supports_parameters('named')) {
                unset($bind[$col]);
                $bind[':col' . $i] = $val;
                $vals[] = ':col' . $i;
                $i++;
        ...
        // build the statement
        $sql = 'INSERT INTO ' . $this->quote_identifier($table, true) . ' (' . implode(', ', $cols) . ') ' . 'VALUES (' . implode(', ', $vals) . ')';
    ...
    public function update($table, array $bind, $where = '')
    {
         ...
         */
        $sql = 'UPDATE ' . $this->quote_identifier($table, true) . ' SET ' . implode(', ', $set) . ($where ? " WHERE {$where}" : '');
        /**
         * Execute the statement and return the number of affected rows
         */
        if ($this->supports_parameters('positional')) {
    ...
    public function delete($table, $where = '')
    {
         ...
         */
        $sql = 'DELETE FROM ' . $this->quote_identifier($table, true) . ($where ? " WHERE {$where}" : '');
        /**
         * Execute the statement and return the number of affected rows
         */
        $stmt = $this->query($sql);
    ...
    public function fetch_all($sql, $bind = [], $fetch_mode = null)
    {
        ...
        $stmt = $this->query($sql, $bind);
        return $stmt->fetch_all($fetch_mode);
    }
    /**
    ...
    {
        $stmt = $this->query($sql, $bind);
        return $stmt->fetch_all(Zend_Db::FETCH_COLUMN, 0);
    }
    /**
    ...
    public function quote($value, $type = null)
    {
        ...
        $value = $value ?? '';
        if ($type !== null && array_key_exists($type = strtoupper($type), $this->_numeric_data_types)) {
            $quoted_value = '0';
            switch ($this->_numeric_data_types[$type]) {
                case Zend_Db::INT_TYPE:
                    // 32-bit integer
                    $quoted_value = (string) (int) $value;
                    break;
     ...
     * @return string The quoted identifier and alias.
     */
    protected function _quote_identifier_as($ident, $alias = null, $auto = false, string $as = ' AS ')
    {
    ...
    {
        if ($auto === false || $this->_auto_quote_identifiers === true) {
    ...
    {
        if ($this->_allow_serialization == false) {
            /** @see Zend_Db_Adapter_Exception */
            #require_once 'Zend/Db/Adapter/Exception.php';
            throw new Zend_Db_Adapter_Exception(get_class($this) . ' is not allowed to be serialized');
        }
        $this->_connection = null;
        return array_keys(array_diff_key(get_object_vars($this), ['_connection' => null]));
    }
    /**
    ...
    {
        if ($this->_auto_reconnect_on_unserialize == true) {
            $this->get_connection();
        }
    }
    /**
     ...
     */
    abstract protected function _roll_back();
    /**
     * Set the fetch mode.
     *
     * @param integer $mode
